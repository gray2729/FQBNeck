...
class FQBNeck(nn.Module):
    def __init__(self, feature_dim=256, latent_dim=128, num_class=2):
        ...
        self.rgb_vib = VIB(feature_dim, latent_dim)
        self.fft_vib = VIB(feature_dim, latent_dim)
    ...
    def forward(self, x):
        ...
        rgb_z, rgb_mu, rgb_logvar = self.rgb_vib(rgb_features)
        fft_z, fft_mu, fft_logvar = self.fft_vib(fft_features)
        
        #features = torch.cat([z, rgb_features], dim=1)
        features = torch.cat([fft_z, rgb_z], dim=1)
        fused_features = self.fuse(features)
        
        logits = self.mlp(fused_features)
        
        #return logits, fft_mu, fft_logvar
        return logits, fft_mu, fft_logvar, rgb_mu, rgb_logvar